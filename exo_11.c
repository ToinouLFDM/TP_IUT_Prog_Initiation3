...
{	
	...
	for (i=0;i< W;i+=20)
	{
		for (j=0;j<H;j+=20)
		{
			...
			for (I=0; I<4;I++)
			{
				...
				{
					tableau[i/20][j/20].wall[I]=entier_aleatoire(50)+1;
				}
			}
		...
		for (i=0;i<H;i+=20)
		{
	...
	
			{
				...
				if (tableau[i/20][j/20].wall[n]==1)
				{
					...
					posWall.x=i;
					posWall.y=j;
					printf("%d,%d,%d  \n",tableau[i/20][j/20].wall[n],posWall.x,posWall.y);
	...
	while (life >=1)
	{
		int I=0;
		clic=attendre_clic();
		printf("Etape 2 %d,%d,%d,%d \n",tableau[player.x/20][player.y].wall[0],tableau[player.x/20][player.y].wall[1],tableau[player.x/20][player.y].wall[2],tableau[player.x/20][player.y].wall[3]);
		int c=(player.x/20)-(clic.x/20) , d=(player.y/20)-(clic.y/20) ;
		int a=fabs(c), b=fabs(d);
		if ( ( (c==1 && (tableau[player.x/20][player.y].wall[1]>1  )&& d==0) || (c==-1 && (tableau[player.x/20][player.y].wall[3]>1) && d==0) || (d==1 && (tableau[player.x/20][player.y].wall[0]>1  )&& c==0) || (d==-1 && (tableau[player.x/20][player.y].wall[2]>1)&& c==0) ) && a!=b  )
